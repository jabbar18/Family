...
function updateUser($user_id, $userName, $password){
	...
	$query = "UPDATE member SET user_name = '$userName', password = '$password' WHERE member_id = $user_id;";
...
function selectUser($userName, $password){
	...
	$query = "SELECT * FROM member WHERE user_name = '$userName' AND password = '$password'";
	$result = mysqli_query($GLOBALS['link'], $query);
	...
	if($result){
		if($row = mysqli_fetch_array($result)){
			$user = array("id"=>$row['member_id'], "username"=>$row['user_name'], "password"=>$row['password']);
		}
	}
